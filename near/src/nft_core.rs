...
impl NonFungibleTokenCore for Contract {
    fn internal_nft_burn(
        ...
        self.internal_remove_token_from_owner(&owner_id.clone(), &token_id);
    }
    fn nft_payout(&self, token_id: String, balance: U128, max_len_payout: Option<u32>) -> Payout {
        let token = self.tokens_by_id.get(&token_id).expect("No such token_id");
        ...
        for (k, v) in self.royalties.iter() {
            ...
            if key != owner_id {
                //
                payout_object.payout.insert(key, royalty_to_payout(*v, balance_u128).into());
                total_perpetual += v;
            }
        }
        // payout to previous owner who gets 100% - total perpetual royalties
        payout_object.payout.insert(owner_id, royalty_to_payout(10000 - total_perpetual, balance_u128).into());
    ...
    fn nft_transfer_payout(
        ...
        self.nft_transfer(receiver_id, token_id, memo);
        payout
    }
    ...
    fn nft_token(&self, token_id: TokenId) -> Option<JsonToken> {
        ...
        if let Some(token) = self.tokens_by_id.get(&token_id) {
            ...
            let mut metadata = self.token_metadata.get().unwrap();
            metadata.title = Some(metadata.title.unwrap_or("".to_string()) + " #" + &token.index.to_string());
            metadata.extra = self.tokens_data.get(&token_id);
